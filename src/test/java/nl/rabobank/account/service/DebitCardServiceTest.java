...
class DebitCardServiceTest {
    ...
    public void withdraw_ok() {
        ...
        AccountEntity accountEntity = getAccountEntity(100.0);
        ...
        AccountEntity accountEntityUpdated = accountEntity.toBuilder().balance(10.0).build();
        ...
        Mono<Void> result = debitCardService.withdraw(withdrawalRequest);
    ...
    public void withdraw_not_enough_balance() {
        ...
        StepVerifier.create(debitCardService.withdraw(withdrawalRequest))
                .expectSubscription()
    ...
    void transfer() {
        ...
        AccountEntity originAccount = getAccountEntity(100.0);
        AccountEntity targetAccount = getAccountEntity(100.0).toBuilder().iban("RABO999").build();
        ...
        AccountEntity originAccountUpdated = originAccount.toBuilder().balance(10.0).build();
        ...
        Mono<Void> result = debitCardService.transfer(transferRequest);
        StepVerifier.create(result).verifyComplete();  // Verifies the sequence completes successfully
    ...
    void transfer_not_enough_balance() {
        ...
        StepVerifier.create(debitCardService.transfer(transferRequest))
                .expectSubscription()
                .expectError(AccountException.class)
                .verify();
    }
    ...
    private CardEntity getCardEntity() {
        return CardEntity.builder().iban("RABO123")
                .type(CardTypeEnum.DEBIT_CARD).number(111L).id(1L)
                .status(CardStatus.ACTIVE).build();
    }

    private AccountEntity getAccountEntity(Double balance) {
        return AccountEntity.builder().id(1L).balance(balance).iban("RABO123").build();
    }
    ...
    private AccountTransactionEntity getWithdrawAccountTransactionEntity(LocalDateTime currentLocalDate) {
        return AccountTransactionEntity
                ...
                .cardType(CardTypeEnum.DEBIT_CARD)
